...
**Un short, 1:20, face caméra.** Croyance contestée : les gummies de créatine, c'est de la créatine en plus pratique. En fait la plupart n'en contiennent presque pas, et c'est encore vendu en toute légalité. La seule façon de savoir : les marques contrôlées par un labo indépendant.
...
Adaptation fidèle de la structure de [[video/creatine-gummies/SCRIPT#0. La référence|SCRIPT § 0]] : le reel `DXZks-ijmmQ` de @trainbloom (90 s, 66K likes, « la plupart des gummies n'ont pas de créatine »), démonté en local le 19 septembre (transcription faster-whisper, Gemini hors service). Ses lignes sont rejouées par toi au lieu du stitch (clip anglais, droits, audience FR). La source de vérité est ce dossier ; le projet HitMe `creatine-gummies` garde le teardown et le bracket.
